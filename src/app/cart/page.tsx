...
import { setCartData, updateAdditionalItemQuantity, removeCartItem, removePackageItems } from '@/store/slices/cartItemsSlice';
...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCartData = async () => {
      try {
        ...
        console.log('Cart ID:', cartData.cart.cartId);
        console.log('User ID:', cartData.cart.userId);
        console.log('Buyer Type:', cartData.cart.buyerType);
        ...
        // Dispatch to Redux store with correct structure
  ...
  const handleProductQuantityChange = async (productId: number, delta: number) => {
    try {
      ...
      const selectedUnit = unitSelection[productId] || 'kg';
      let basePrice = 0;
      let baseDiscount = 0;
      ...
      const newTotalPrice = calculatePrice(basePrice, selectedUnit, newQuantity);
      const newTotalDiscount = calculateDiscount(baseDiscount, selectedUnit, newQuantity);
      
      // Dispatch to Redux store
      dispatch(updateAdditionalItemQuantity({
        itemId: productId,
        newQuantity,
        newTotalPrice,
        newTotalDiscount,
      ...
      setSummary(updatedCartData.summary);
      dispatch(setCartData({
        cartId: updatedCartData.cart.cartId,
  ...
  const handleRemovePackage = async (packageId: number) => {
    try {
      ...
      // Dispatch to Redux store
      dispatch(removePackageItems(packageId));
  ...
  const handleCheckout = () => {
    ...
    try {
      // Redux store already has the cart data, just navigate
