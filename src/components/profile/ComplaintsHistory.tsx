...
const ComplaintsHistory = () => {
  ...
  const [filter, setFilter] = useState<'This month' | 'Last month' | 'All'>('This month');
  ...
  return (
    <div className={`relative z-10 px-4 sm:px-6 md:px-8 min-h-screen mb-10 ${selectedComplaint ? 'bg-white' : 'bg-white'}`}>
      <h2 className="font-medium text-base sm:text-lg md:text-xl mb-2 mt-2">
        Complaints History
      </h2>
      <p className="text-xs sm:text-sm text-[#626D76] mb-3">
        ...
        <div className="relative z-25">
          <div className="flex flex-row justify-between items-center mb-4 sm:mb-6">
            ...
            <div className="relative w-32 sm:w-40">
              <select
                value={filter}
                onChange={(e) => setFilter(e.target.value as 'This month' | 'Last month' | 'All')}
          ...
          {error && <div className="text-center text-sm text-red-600">Error: {error}</div>}
          {!loading && !error && filteredComplaints.length === 0 && (
            <div className="text-center text-sm text-[#626D76]">No complaints found.</div>
          )}
                ...
                <div key={complaint.id} className="border border-[#CECECE] rounded-lg p-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                    <div className="flex flex-col items-start">
                      <div className="text-xs sm:text-sm text-[#626D76] font-medium">Complaint ID:</div>
                      <div className="text-xs sm:text-sm">#ICustomerID-{complaint.id}</div>
                    </div>
                    <div className="flex flex-col items-start">
                      <div className="text-xs sm:text-sm text-[#626D76] font-medium">Category:</div>
                      <div className="text-xs sm:text-sm">{complaint.category}</div>
                    </div>
                    <div className="flex flex-col items-start">
                      <div className="text-xs sm:text-sm text-[#626D76] font-medium">Date:</div>
                      <div className="text-xs sm:text-sm">{complaint.date}</div>
                    </div>
                    <div className="flex flex-col items-start sm:items-center">
                      <div className="flex items-center">
                        <span
                          className={`min-w-[100px] sm:min-w-[120px] text-center px-2 py-1 rounded-full text-xs sm:text-sm ${
                            complaint.status === 'Closed'
                              ? 'bg-[#3E206D]/10 text-[#3E206D]'
                    ...
                    <div className="flex flex-col items-start sm:items-end">
                      <div className="flex items-center">
                        <button
                          ...
                          className="w-20 sm:w-24 h-8 sm:h-9 text-xs sm:text-sm rounded-lg text-white bg-[#3E206D] hover:bg-[#341a5a] -mt-1 mr-12"
                        >
                          View Reply
                        </button>
                      </div>
                  ...
                  <p className="text-xs sm:text-sm text-[#626D76] mb-3">{complaint.description}</p>
                  <div className="flex gap-2 flex-wrap">
                    {complaint.images.length > 0 ? (
                      ...
                      <p className="text-xs sm:text-sm text-[#626D76]">No images available.</p>
                    )}
                  </div>
                </div>
              ))}
