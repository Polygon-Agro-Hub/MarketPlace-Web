    ...
    const fetchItems = async () => {
      ...
      try {
        ...
        const suggestionsRes =
          await getMarketplaceSuggestionsProfile(authToken);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setExcludedItems((prev) =>
        prev.filter((item) => !itemsToDelete.includes(item.displayName)),
      );
      setSelectedItems((prev) =>
    ...
    <div className="relative z-10 px-4 sm:px-6 md:px-8 min-h-screen bg-white blur-effect py-6">
      ...
      <div className="flex items-center justify-center bg-white">
        <div className="w-full max-w-xl mx-auto">
          ...
          {loading && (
            <p className="text-center text-[#4C5160] text-sm md:text-base">
              Loading...
            </p>
          )}
          {error && (
